...
public:
    ...
    void fillDistance(int r, int c, int landFlag, vector<vector<int>>& grid, vector<vector<vector<int>>>& sums) {
        queue<pair<int, int>> q;
        vector<vector<bool>> visited(rows, vector<bool>(cols));
        int cnt = 0;
    ...
    int shortestDistance(vector<vector<int>>& grid) {
        ...
        for (int i = 0; i < rows; i++) {
            for (int j = 0; j < cols; j++) {
                if (grid[i][j] == 1) {
                    fillDistance(i, j, landFlag, grid, sums);
                    numberOfBuilding++;
                    landFlag--;
                }
            }
        ...
        for (int i = 0; i < rows; i++) {
            for (int j = 0; j < cols; j++) {
                if (sums[i][j][1] == numberOfBuilding) {
                    ret = min(ret, sums[i][j][0]);
                }
            }
        }
